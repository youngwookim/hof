...
public class MySslConfiguration implements SslConfiguration {
  ...
  /**
   * Set what client authentication level to use, supported values are "yes" or
   * "true" for required authentication, "want" for wanted authentication and
   * "false" or "none" for no authentication. Defaults to "none".
   *
  ...
  /**
   * Set the allowed cipher suites, note that the exact list of supported cipher
   * suites differs between JRE implementations.
   *
   ...
   * specified key store contains multiple keys, this alias can be set to select
   * a specific key.
   *
   * @param keyAlias The alias to use, or null if JSSE should be allowed to
   * choose the key.
   */
